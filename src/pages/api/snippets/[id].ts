...
import type { GetSnippetByIdCommand, DeleteSnippetCommand } from '@/types/snippet.dto';
 ...
 * Delete a specific snippet by its ID for the authenticated user
 */
export const DELETE: APIRoute = async (context) => {
  try {
    ...
    // 4. Build service command
    const command: DeleteSnippetCommand = {
      id: validation.data.id,
      user_id,
    };

    // 5. Call service
    const result = await SnippetService.deleteSnippet(command, access_token);
